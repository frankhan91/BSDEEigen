...
class DoubleWell_d5Eigen(Equation):
    ...
    def __init__(self, eqn_config):
        ...
        self.N = 10
        self.sigma = np.sqrt(2.0)
        self.A = [1.5,0.2,0.2,0.2,0.2]
        self.true_eigen = -0.28585620906438886
        self.second_eigen = 0.16603814620131496
        # coef0 is coef for ground eigenfunction: 1, cos2x, cos4x...
        self.coef0 = [[4.849638936137847245e-01,4.996884608914199388e-01,4.996884608914199388e-01,4.996884608914199388e-01,4.996884608914199388e-01],
                      [-1.719216620376243232e-01,-2.495715904542338340e-02,-2.495715904542338340e-02,-2.495715904542338340e-02,-2.495715904542338340e-02],
                      [7.934668280954022823e-03,1.559362739878917203e-04,1.559362739878917203e-04,1.559362739878917203e-04,1.559362739878917203e-04],
                      [-1.641332919106110351e-04,-4.330981093820056683e-07,-4.330981093820056683e-07,-4.330981093820056683e-07,-4.330981093820056683e-07],
                      [1.915646686059286880e-06,6.766640464567428604e-10,6.766640464567428604e-10,6.766640464567428604e-10,6.766640464567428604e-10],
                      [-1.432980906804100879e-08,-6.766307217324653586e-13,-6.766307217324653586e-13,-6.766307217324653586e-13,-6.766307217324653586e-13],
                      [7.449835308605464979e-11,4.698663151849139795e-16,4.698663151849139795e-16,4.698663151849139795e-16,4.698663151849139795e-16],
                      [-2.846866874407665454e-13,-2.397216509273572312e-19,-2.397216509273572312e-19,-2.397216509273572312e-19,-2.397216509273572312e-19],
                      [8.247577849078903519e-16,9.363952505944370821e-23,9.363952505944370821e-23,9.363952505944370821e-23,9.363952505944370821e-23],
                      [-1.840772951476521254e-18,3.060820295240397227e-26,3.060820295240397227e-26,3.060820295240397227e-26,3.060820295240397227e-26]]
    ...
    def true_y(self, x):
        bases_cos = x*0
        for m in range(self.N):
            bases_cos = bases_cos + tf.cos(2*m*x) * self.coef0[m] #broadcast
        return tf.reduce_prod(bases_cos, axis=1, keepdims=True)
    ...
    def second_y(self, x):
        ...
        bases_sin = x[:,0:1]*0
        for m in range(self.N):
            bases_cos = bases_cos + tf.cos(2*m*x[:,1:5]) * self.coef0[m][1:5] #broadcast
            bases_sin = bases_sin + tf.sin((2*m+1) * x[:,0:1]) * self.coef1[m]
        return tf.reduce_sum(bases_cos, axis=1, keepdims=True) * bases_sin
    ...
    def second_z(self, x):
        ...
        for m in range(self.N):
            y1 = y1 + tf.sin((2*m+1) * x[:,0:1]) * self.coef1[m]
            y2 = y2 + tf.cos(2 * m * x[:,1:5]) * self.coef0[m][1:5]
            dy1 = dy1 + (2*m+1) * tf.cos((2*m+1) * x[:,0:1]) * self.coef1[m]
            dy2 = dy2 - 2*m * tf.sin(2*m * x[:,1:5]) * self.coef0[m][1:5]
        y = tf.reduce_sum(y2, axis=1, keepdims=True) * y1
